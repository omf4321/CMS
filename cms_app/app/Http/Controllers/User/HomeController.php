...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $echelon_array = \App\Model\Admin\subject::where('teacher_id', $teacher->id)->pluck('echelon_id')->toArray();;
        $echelons = Cache::rememberForever("echelons_" . $user->id, function () use ($echelon_array) {
            $result = \App\Model\Admin\echelon::whereIn('id', $echelon_array)->get();
            return $result;
        }); 
    ...
    public function test() {
        $user = Client::find(2);
        $message = daily_schedule::first();
        $message->online_class_status = 'begin';
