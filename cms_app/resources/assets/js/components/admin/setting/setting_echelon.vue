...
<template>
   <div>
      <v-container fluid>
         <!-- @page_headline -->
         <h3 class="headline">Manage Class</h3>
              ...
              >
               ...
               <v-toolbar flat color="white">
                  ...
                  <v-dialog v-model="dialog" persistent max-width="500px">
                    <!-- @add_item_button -->
                     <v-btn slot="activator" color="primary" dark class="mb-2" @click="editedIndex=-1, get_branch()">New Class</v-btn>
                     <v-card>
                        <v-card-title>
                          <!-- @add_item_title -->
               ...
               <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                     ...
                     <td class="text-xs-left">{{ props.item.class_no }}</td>
                     ...
                     <td class="justify-center layout px-0">
                        ...
                        <v-icon
                           small
                           @click="admin_echelon_delete(props.item)"
                           >
                           delete
                        </v-icon>
                     </td>
...
<script>
   export default {
     ...
     methods:{
       ...
       admin_echelon_list(){
         this.data_load = true;
         this.axios.get('/admin/request/admin_echelon_list').then(response => {
             this.rows = response.data.admin_echelon_list;
       ...
       // @delete_item_method
       admin_echelon_delete(item){
         var con = confirm("Want to delete?");
         if (con) {
            const index = this.rows.findIndex(x => x.id==item.id);
            this.axios.post('/admin/request/admin_echelon_delete/'+item.id).then(response => {
