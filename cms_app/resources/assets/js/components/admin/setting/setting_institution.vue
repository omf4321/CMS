...
<template>
   <div>
      <v-container fluid>
         <!-- @page_headline -->
         <h3 class="headline">Manage Institution</h3>
         <v-divider class="my-3"></v-divider>
         <template>
            <div>              
               <v-toolbar flat color="white">
                  <!-- @list_title -->
                  <v-toolbar-title>Institution List</v-toolbar-title>
                  <v-spacer></v-spacer>
                  <v-text-field
                    v-model="search"
                    append-icon="search"
                  ...
                  <v-dialog v-model="dialog" persistent max-width="500px">
                    <!-- @add_item_button -->
                     <v-btn slot="activator" color="primary" dark class="mb-2" @click="editedIndex=-1, get_branch()">New Institution</v-btn>
                     <v-card>
                        <v-card-title>
                          <!-- @add_item_title -->
               ...
               <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                     ...
                     <td class="justify-center layout px-0">
                        ...
                        <v-icon
                           small
                           @click="admin_institution_delete(props.item)"
                           >
                           delete
                        </v-icon>
                     </td>
...
<script>
   export default {
     ...
     beforeRouteEnter (to, from, next) {
          let p = window_admin_permissions.findIndex(x => x.name=='setting_institution');
     ...
     computed: {
       formTitle () {
         return this.editedIndex === -1 ? 'New Institution' : 'Edit Institution'
       }
     },
     ...
     methods:{
       ...
       admin_institution_list(){
         this.data_load = true;
         this.axios.get('/admin/request/admin_institution_list').then(response => {
             this.rows = response.data.admin_institution_list;
       ...
       admin_institution_add(){
          if (this.$refs.form.validate()) {
            this.loading=true;
            this.axios.post('/admin/request/admin_institution_add', this.dialogItem).then(response => {
                this.admin_institution_list();
                this.close()
       ...
       // @edit_item_method
       admin_institution_edit(){
          this.loading=true;
          this.axios.post('/admin/request/admin_institution_edit/'+this.editedId, this.dialogItem).then(response => {
       ...
       save(){
          if (this.editedIndex > -1) {
            this.admin_institution_edit()
          } else {
            this.admin_institution_add()
          }
       },
       // @delete_item_method
       admin_institution_delete(item){
         var con = confirm("Want to delete?");
         if (con) {
            const index = this.rows.findIndex(x => x.id==item.id);
            this.axios.post('/admin/request/admin_institution_delete/'+item.id).then(response => {
