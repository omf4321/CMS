...
<template>
    <div>
        <v-container fluid>
            <!-- @page_headline -->
            <h3 class="headline">Manage Funds</h3>
            <v-divider class="my-3"></v-divider>
            <template>
                <div>
                    <v-toolbar flat color="white">
                        <!-- @list_title -->
                        <v-toolbar-title>Fund List</v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <v-dialog v-model="dialog" persistent max-width="500px">
                            <!-- @add_item_button -->
                            <v-btn slot="activator" color="primary" dark class="mb-2" @click="editedIndex=-1, get_branch()">New Fund</v-btn>
                            <v-card>
                                <v-card-title>
                                    <!-- @add_item_title -->
                    ...
                    <v-data-table :headers="headers" :items="rows" :search="search" :loading="data_load" hide-actions class="elevation-1">
                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td class="justify-center layout px-0">
                                ...
                                <!-- @delete_item -->
                                <v-icon small @click="delete_fund(props.item)">
                                    delete
                                </v-icon>
                            </td>
                        </template>
...
<script>
    export default {
        data() {
                return {
                    // @list_header_data
                    headers: [{text: 'ID', align: 'left', value: 'id'}, {text: 'Name', value: 'name'}, {text: 'Description', value: 'description'}, {text: 'Created At', value: 'created_at'}, {text: 'Actions', value: 'name', sortable: false, align: 'center'}],
                    // @list_column_data
                    rows: [],
                    branch: [],
                    // @list_search_data
            ...
            computed: {
                formTitle() {
                    return this.editedIndex === -1 ? 'New Fund' : 'Edit Fund'
                }
            },
            ...
            methods: {
                ...
                get_fund_list() {
                    ...
                    Vue.axios.get('/admin/transaction/get_fund_list').then(response => {
                        this.rows = response.data.fund_list;
                ...
                add_fund() {
                    if (this.$refs.form.validate()) {
                        this.loading = true;
                        Vue.axios.post('/admin/transaction/add_fund', this.dialogItem).then(response => {
                            this.rows.push(response.data.fund);
                            this.close()
                ...
                edit_fund() {
                    this.loading = true;
                    Vue.axios.post('/admin/transaction/edit_fund/' + this.editedId, this.dialogItem).then(response => {
                ...
                delete_fund(item) {
                    ...
                    if (con) {
                        ...
                        Vue.axios.post('/admin/transaction/delete_fund/' + item.id).then(response => {
