...
<template>
    <div>
        <v-container fluid>
            <!-- @page_headline -->
            <h3 class="headline">Manage Group</h3>
                ...
                <div>
                    <v-toolbar flat color="white">
                        ...
                        <v-dialog v-model="dialog" persistent max-width="500px">
                            <!-- @add_item_button -->
                            <v-btn slot="activator" color="primary" dark class="mb-2" @click="editedIndex=-1, get_branch()">New group</v-btn>
                            <v-card>
                                <v-card-title>
                                    <!-- @add_item_title -->
                    ...
                    <v-data-table :headers="headers" :items="rows" :search="search" :loading="data_load" hide-actions class="elevation-1">
                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td class="justify-center layout px-0">
                                ...
                                <!-- @delete_item -->
                                <v-icon small @click="admin_group_delete(props.item)">
                                    delete
                                </v-icon>
                            </td>
                        </template>
    ...
    export default {
        data() {
            ...
            methods: {
                ...
                admin_group_list() {
                    this.data_load = true;
                    this.axios.get('/admin/request/admin_group_list').then(response => {
                        this.rows = response.data.admin_group_list;
                ...
                admin_group_add() {
                    if (this.$refs.form.validate()) {
                        this.loading = true;
                        this.axios.post('/admin/request/admin_group_add', this.dialogItem).then(response => {
                            this.admin_group_list();
                            this.close()
                ...
                admin_group_edit() {
                    this.loading = true;
                    this.axios.post('/admin/request/admin_group_edit/' + this.editedId, this.dialogItem).then(response => {
                ...
                save() {
                    if (this.editedIndex > -1) {
                        this.admin_group_edit()
                    } else {
                        this.admin_group_add()
                    }
                },
                get_branch() {
                    if (this.branch.length == 1) {
                ...
                // @delete_item_method
                admin_group_delete(item) {
                    var con = confirm("Want to delete?");
                    if (con) {
                        const index = this.rows.findIndex(x => x.id == item.id);
                        this.axios.post('/admin/request/admin_group_delete/' + item.id).then(response => {
